...
let skyColor = [234, 198, 136];
let grassColor = [170, 120, 31];
let dirtColor = [132, 86, 24];
let stoneColor = [80, 52, 2];
let ironColor = [44, 44, 20];
let diamondColor = [184, 47, 1];
let playerColor = [20, 21, 15];
...
// draws the player and updates the inventory
function draw() {
  doPlayerMovement();
  updatePlayerInventory();
}

// adds event listeners to the the material containers
...
// sets the container that the player has currently selected
...
// restarts the game with a new seed
function restart(){
  let url = window.location.href;
  window.location.href = url;
}

// sets the seed. as of right now, this seed does not get used
// if you wanted to be able to enter a custom seed, you could change the assignment of seed and uncomment the lines below it
function setSeed(){
...
// procedurally generates the terrain
...
// allows player to jump when up arrow or W key is pressed
function doPlayerJump(){
  for(let i = 0; i < JUMP_HEIGHT; i++){
    if((keyIsDown(UP_ARROW) || keyIsDown(87)) && findNode(player.x, player.y - BLOCK_SIZE).blockType === sky){
...
// allows player to fall if they are in the air
...
// allows player to move right when the right arrow or D key is pressed
function doPlayerMoveRight(){
  if(player.x + BLOCK_SIZE > WIDTH){
    return;
  }
  if((keyIsDown(RIGHT_ARROW) || keyIsDown(68)) && findNode(player.x + BLOCK_SIZE, player.y).blockType === sky){
    updatePlayerPosition(player.x + BLOCK_SIZE, player.y);
  }
}

// allows player to move left when the left arrow or A key is pressed
function doPlayerMoveLeft(){
  if(player.x - BLOCK_SIZE < 0){
    return;
  }
  if((keyIsDown(LEFT_ARROW) || keyIsDown(65)) && findNode(player.x - BLOCK_SIZE, player.y).blockType === sky){
    updatePlayerPosition(player.x - BLOCK_SIZE, player.y);
  }
}

// finds the node at the entered x and y coordinates
...
// sets the position that the player was previously at to a sky block
function setPreviousPlayerPosition(){
  fill(skyColor);
  rect(player.x, player.y, BLOCK_SIZE, BLOCK_SIZE);
}

// sets the new position that the player is going to
function setNewPlayerPosition(){
  fill(playerColor);
  rect(player.x, player.y, BLOCK_SIZE, BLOCK_SIZE);
}

// allows a mouse press to interact with the blocks
...
// determines if the player can interact with the clicked block
function playerCanInteractWithBlock(node){
  let distance = dist(node.x, node.y, player.x, player.y);
  return ((distance <= BLOCK_SIZE * 1.5) && distance != 0);
}

// allows player to mine or place a block
...
// adds a mined block to the player's inventory
...
// gets the color of the entered block
...
// makes sure player has enough resources to place a block
